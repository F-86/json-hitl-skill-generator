...
| `type` | string | 是 | 交互类型：`choice` / `confirm` / `review` / `input` / `text_input` / `combobox` / `number_range` / `datetime_range` |
...
| `risk` | string | 否 | 风险标记：`"safe"`（安全，不触发后续写操作）或 `"dangerous"`（会触发后续写操作）。用于前端硬护栏：当前置 apicall 失败时，过滤 `risk: "dangerous"` 的选项 |
...
**使用条件：** 仅限 Create / Update 场景，用户需要填写新数据（如商品名称、价格等）。**禁止**用于查询条件收集——查询场景中需要自由文本输入的字段（如 id、名称）使用 `text_input` 类型。
...
用户在单行文本框中输入自由文本，适用于查询场景中需要手动输入的字段（如 id、名称等）。

**使用条件：** 查询条件中，字段值不由后端枚举（否则用 `combobox`），也不是数值/时间范围（否则用 `number_range`/`datetime_range`），而是用户手动输入的文本（如 id 列表、名称关键词）。
...
| `field` | 是 | 对应资源的字段名，agent 用于构建 filters key |
| `label` | 是 | 展示给用户的字段名称 |
| `placeholder` | 否 | 输入框占位提示文本 |
| `default` | 否 | 默认预填值（LLM 从用户输入提取后填入） |

**多值处理：** 当字段支持多个值时（如多个 id、多个名称），`placeholder` 应说明分隔符（如"多个用逗号分隔，中英文逗号均可"）。前端负责将逗号分隔的字符串解析为数组。
...
| 10 | `type` 为 `choice`/`confirm`/`review`/`input`/`text_input`/`combobox`/`number_range`/`datetime_range` | 🔴 必须 |
...
| 17 | `type=text_input` 时 `field`、`label` 均存在 | 🔴 必须 |
| 18 | `option.risk` 若存在，必须为 `"safe"` 或 `"dangerous"` | 🟡 建议 |
...
|------|------------|------|--------------|
| Read/Query | 内嵌在条件收集 `hitl` 中 | `checkpoint.apicall` 字段，`filters: {}` 由前端提交时填充 | `POST /api/<resource>/query`，body 为 `{"filters": {<field>: [<values>]}}` |
...
### 多步流程状态机（dry_run / 预览场景）

部分操作（如批量 Update）采用「前置预览 apicall → 确认 hitl → 正式执行 apicall」多步流程。**前置 apicall（dry_run/预览）本身也可能失败**（4xx/409，如批内重名、0 条命中、与库内冲突）。

**状态机规则：**

| 前置 apicall 结果 | 后续允许输出 | 后续禁止输出 |
|------------------|------------|------------|
| 成功 | 确认 hitl 块 + 正式执行 apicall | — |
| 失败（任意 4xx/5xx） | 询问类 hitl（input 块，告知报错并请用户调整） | 确认 hitl 块（如 approve/refine/cancel）、正式执行 apicall |

> **🔴 铁律**：前置 apicall 失败时，LLM **绝对不要**继续输出确认类 hitl 块或正式执行 apicall。应改输出询问类 hitl，告知用户报错原因并请其调整参数/表达式/路径。
>
> **注意**：这是 prompt 层软约束，LLM 仍可能违规。消费侧（前端）必须实现硬护栏：当前置 apicall 结果为 error 时，过滤同消息内后续 hitl 中 `risk: "dangerous"` 的选项（或硬编码过滤 approve/confirm/execute 类 value）。
...
| 1.2 | — | 新增 `option.risk` 字段（safe/dangerous）；新增多步流程状态机规则（前置 apicall 失败阻断） |
| 1.3 | 2026-06-24 | 新增多步流程的结构化回执归一化建议、自然语言消息分流规则，以及 ` ```text ` 误包裹 runtime JSON 的消费侧兼容建议；放置规则补齐批量 Delete |
