...
> **多任务类型场景：** 如果用户说"我有 N 种任务（如订单、用户、任务）都需要增删改查"，按以下流程处理：
> 1. 优先问清楚是哪几种任务类型，列出清单让用户确认
> 2. 对每种任务类型走 CRUD 批量模式，按 Day1 原型 → Day2 批量的节奏依次进行
> 3. 建议先完成第一种任务类型的全部 4 个 skill，获得确认后再开始下一种
> 4. 全部完成后，汇总交付路径，提交用户最终确认
...
3. **CRUD 操作说明**：每种操作具体做什么？
   ...
   - Update：哪些字段可改？
   - Delete：什么条件下能删？
4. **操作参数的详细结构**：每种操作需要哪些参数。对每个参数，标注：
   - **确定性**：LLM 能直接从用户输入中提取 → 不触发 HITL
   - **可能缺失**：用户可能不说但操作必须 → 触发 HITL（`input` 类型）
   - **可能歧义**：用户说了但含义模糊 → 触发 HITL（`choice` 类型）
   
   > 这是后续生成 HITL 触发条件表的核心输入。必须逐参数标注，不能笼统说"有些不确定"。
   > Read/Query 参数在这一轮问清楚；Create/Update/Delete 的参数可在第二轮追问。
...
4. **HITL 触发条件提取**（新增）：
   ...
   | 类别 | 判断标准 | HITL 行为 |
   |------|---------|----------|
   | **确定性参数** | LLM 100% 能从用户输入中直接提取 | 不触发 HITL |
   | **可能缺失参数** | 用户可能没说，但操作必须 | 触发 HITL（`input` 类型） |
   | **歧义参数** | 用户说了但含义模糊（"最近的"、"那个"） | 触发 HITL（`choice` 类型） |

   判断方法：回想用户描述中提到的"哪些参数 LLM 可能不确定"（Phase 0 收集的信息），将其映射为具体的触发条件。

5. **触发条件映射规则**（新增）：
   将第 4 步的分类结果映射到生成的 skill 的 HITL 触发条件表中：

   | 参数类别 | 是否出现在触发条件表中 | 模板行内容 | HITL 块类型 |
   |---------|---------------------|-----------|------------|
   | **确定性参数** | ❌ 不出现 | 不放行（LLM 自动处理，不写入触发条件表） | — |
   | **可能缺失参数** | ✅ 出现 | `| <参数名> 缺失且必填 | ✅ | input | 收集缺少的必填参数 |` | `input` |
   | **歧义参数** | ✅ 出现 | `| <参数名> 存在歧义（如"最近的"） | ✅ | choice | 让用户明确选择 |` | `choice` |

   > 映射时注意：
   > - 每个"可能缺失"参数生成一行触发条件，对应到 Phase 中一个 `input` 类型的 HITL 块
   > - 每个"歧义参数"生成一行触发条件，对应到 Phase 中一个 `choice` 类型的 HITL 块
   > - 确定性参数不写入触发条件表（表中只列需要触发 HITL 的条件；"全部参数齐全"和"用户明确跳过"两行是固定的，始终保留）
   > - 最终确认由 CRUD 差异矩阵决定，不属于此映射规则
...
3. **CRUD 差异表**
4. **HITL 触点地图 × 4**
5. **HITL 触发条件矩阵**（新增）：对每种操作，列出确定性参数 / 可能缺失参数 / 歧义参数三类，明确每个触发条件对应的 HITL 块类型
6. **原型选择**：推荐从 Read（最简单）或 Create（最常见）开始
...
6. **HITL 触发条件矩阵完整？** 每种操作的参数都分了确定性 / 可能缺失 / 歧义三类？
...
- [ ] HITL 触发条件表存在且完整（参数触发矩阵 + 最终确认触发条件）
