...
| 模式 | 输出 | 适用场景 |
|------|------|---------|
| **标准模式** | 1 个 SKILL.md | 单个独立的 skill |
| **CRUD 批量模式** | 4 个 SKILL.md（增-查-改-删） | 同一任务类型的多操作场景 |
...
1. **````hitl` 围栏转义错误**：外层围栏和内部 ```hitl 块的关系必须正确。
2. **没有引导语就扔出 JSON 块**：必须附带自然语言说明。
3. **Checkpoint 过多**：超过 4 个 CP 会让用户烦躁。Read 只有 2 个 CP。
4. **CRUD 模式：把 Read 也加了 final confirm**：查是安全的，不需要。
5. **CRUD 模式：Delete 的 final confirm 设了全局 default**：禁止，必须让用户主动选择。
6. **决策选项语义模糊**：`"你觉得怎么样？"` 不是有效选项。
7. **忘记生成决策收集铁律**：每个 CP 处必须写明。
8. **生成的 skill 没有边界条件**：必须包含判断段。
9. **CRUD 命名不一致**：必须统一 `<task-type>-<operation>`。
10. **CRUD 模式跳过原型环节**：必须走 Day 1 原型 → 确认 → Day 2 批量。

11. **Generated skill 对话示例中混合 fence 嵌套**：在原型/生成的 skill 的对话示例章节中，如果使用了 ```` 围栏包裹对话文本，其内部不要再嵌入 ````hitl` 块。这会导致 Markdown 解析器和 JSON 校验器误匹配 fence。改用 `>` 块引用 + 独立 ````hitl` 块，或用 ````text` 加自然语言说明来展示示例 JSON。
